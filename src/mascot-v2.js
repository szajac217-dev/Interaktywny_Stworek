(function (global) {
  ...
  const REACTIONS = {
    ...
    rodzinne: { type: 'center', state: 'happy', bubble: '🥳', fraction: 0.44, duration: 2800 },
    kino: { type: 'center', state: 'look', bubble: '🎬', fraction: 0.65, duration: 2400 },
    sport: { type: 'climb', side: 'left', bubble: '🤯' },
    warsztaty: { type: 'lean', side: 'left', variant: 'hand', bubble: '💡' },
    teatr: { type: 'center', state: 'point', bubble: '👏', fraction: 0.58, duration: 2600 },
  ...
  class InteractiveMascot {
    constructor(options = {}) {
      ...
      this.build();
      this.measure();
      this.placeImmediately({ zone: this.options.startZone, fraction: this.options.startFraction, stance: 'ground' });
      this.setState('idle');
      this.bind();
    }
    ...
    bind() {
      if (this.options.listenWindowEvents) global.addEventListener('signage:poster-change', this.onPosterEvent);
      if ('ResizeObserver' in global) {
        this.resizeObserver = new ResizeObserver(() => {
          this.measure();
          this.placeImmediately({ left: this.currentLeft, bottom: this.currentBottom, useAbsolute: true });
        });
    ...
    setState(state) {
      [...this.element.classList].filter((name) => name.startsWith('state-')).forEach((name) => this.element.classList.remove(name));
    ...
    resolvePosition({ zone = 'track', fraction = 0.5, stance = 'ground' } = {}) {
      const rect = this.zoneRects[zone] || { left: 0, top: 0, width: this.hostWidth, height: this.hostHeight, bottom: this.hostHeight };
      const normalized = clamp(Number(fraction) || 0, 0, 1);
      const leftRange = Math.max(0, rect.width - this.mascotWidth * 0.84);
      const left = clamp(rect.left + leftRange * normalized, -this.mascotWidth * 0.2, this.hostWidth - this.mascotWidth * 0.55);
      const bottom = stance === 'top' ? this.hostHeight - rect.top - this.mascotWidth * 0.18 : this.hostHeight - rect.bottom - 2;
      return { left, bottom };
    }

    placeImmediately(target) {
      const position = target.useAbsolute ? target : this.resolvePosition(target);
    ...
    async moveTo(target, duration = 1400, state = 'walk', token = this.token) {
      const position = target.useAbsolute ? target : this.resolvePosition(target);
      this.face(position.left < this.currentLeft ? 'left' : 'right');
      this.setState(state);
      this.element.style.transitionDuration = `${duration}ms, ${Math.min(900, duration)}ms`;
      this.element.style.left = `${position.left}px`;
      this.element.style.bottom = `${position.bottom}px`;
      this.currentLeft = position.left;
      this.currentBottom = position.bottom;
    ...
    async performCenterReaction(config, token) {
      const arrived = await this.moveTo({ zone: 'track', fraction: config.fraction || 0.5, stance: 'ground' }, 1350, 'walk', token);
      if (!arrived) return;
      await Promise.all([this.say(config.bubble, 1500, token), this.perform(config.state, config.duration || 2400, token)]);
    ...
    async leanOn(side = 'left', variant = 'hand', token = ++this.token, bubble = '👀') {
      const zone = side === 'left' ? 'leftQr' : 'rightQr';
      const fraction = side === 'left' ? 0.78 : 0.08;
      const arrived = await this.moveTo({ zone, fraction, stance: 'ground' }, 1250, 'walk', token);
      if (!arrived) return;
      ...
      await this.say(bubble, 1600, token);
      if (token !== this.token) return;
      await wait(700);
      if (token === this.token) this.setState('idle');
    }
    ...
    async climb(side = 'left', token = ++this.token, bubble = '😄') {
      const zone = side === 'left' ? 'leftQr' : 'rightQr';
      const approachFraction = side === 'left' ? 0.86 : 0.04;
      const arrived = await this.moveTo({ zone, fraction: approachFraction, stance: 'ground' }, 1150, 'walk', token);
      if (!arrived) return;
      this.setTargetSide(side);
      this.face(side === 'left' ? 'left' : 'right');
      const climbed = await this.moveTo({ zone, fraction: 0.12, stance: 'top' }, 900, 'climb', token);
      if (!climbed) return;
      this.face(side === 'left' ? 'right' : 'left');
      this.setState('look');
      await this.say(bubble, 1500, token);
      if (token !== this.token) return;
      this.setState('happy');
      await wait(1300);
      if (token === this.token) this.setState('idle');
    ...
    async react(input = {}) {
      ...
      this.element.classList.remove('is-speaking');
      this.element.dispatchEvent(new CustomEvent('mascot:reaction-start', { bubbles: true, detail: { category, input } }));

      if (config.type === 'climb') await this.climb(config.side, token, input.bubble || config.bubble);
      else if (config.type === 'lean') await this.leanOn(config.side, config.variant, token, input.bubble || config.bubble);
      else await this.performCenterReaction({ ...config, bubble: input.bubble || config.bubble }, token);

      if (token !== this.token) return;
      this.element.dispatchEvent(new CustomEvent('mascot:reaction-end', { bubbles: true, detail: { category, input } }));
    }
